...
func (mock *MockWorkingDirLocker) TryLock(repoFullName string, pullNum int, projectPath string, workspace string) (func(), error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockWorkingDirLocker().")
	}
	params := []pegomock.Param{repoFullName, pullNum, projectPath, workspace}
...
func (verifier *VerifierMockWorkingDirLocker) TryLock(repoFullName string, pullNum int, projectPath string, workspace string) *MockWorkingDirLocker_TryLock_OngoingVerification {
	params := []pegomock.Param{repoFullName, pullNum, projectPath, workspace}
...
func (c *MockWorkingDirLocker_TryLock_OngoingVerification) GetCapturedArguments() (string, int, string, string) {
	repoFullName, pullNum, projectPath, workspace := c.GetAllCapturedArguments()
	return repoFullName[len(repoFullName)-1], pullNum[len(pullNum)-1], projectPath[len(projectPath)-1], workspace[len(workspace)-1]
}
